    ...
    <body class="font-sans selection:bg-stone-500 selection:text-white" style="background-color: #E1DDD9; antialiased">
        <div class="min-h-screen flex flex-col sm:justify-center items-center px-10 mx-auto p-0">
            ...
            <div class="w-full sm:max-w-md mt-8 px-8 py-4 bg-orange-900 shadow-md overflow-hidden rounded-lg">
                <form method="POST" action="{{ route('contentCatalog.store') }}" enctype="multipart/form-data">
                    @csrf

                    <!-- Product Image -->
                    <div class="mt-4">
                        <x-input-label for="imageCtg" :value="__('Image Catalog')" />
                        <x-text-input id="imageCtg" class="block mt-1 w-full" type="file" accept="imageCtg/*" name="imageCtg" :value="old('imageCtg')" required autofocus autocomplete="name" />
                        <x-input-error :messages="$errors->get('imageCtg')" class="mt-2" />
                    </div>

                    <div class="flex items-center justify-end mt-4">
                        <!-- Button Back ke Tabel Content Catalog -->
                        <a class="underline text-sm text-gray-300 hover:text-gray-100 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-orange-950" href="content-catalog">
                            Back
                        </a>
